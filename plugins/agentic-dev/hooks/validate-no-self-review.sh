...
_hook_log() {
  ...
  printf '%s\thook=validate-no-self-review\taction=%s\t%s\n' \
...
if echo "$COMMAND" | grep -qE 'gh\s+pr\s+review' && ! echo "$COMMAND" | grep -q 'codex-review'; then
  _hook_log "blocked" "reason=direct-pr-review cmd=$_CMD_SHORT"
...
if echo "$COMMAND" | grep -qE 'gh\s+pr\s+comment'; then
  _hook_log "blocked" "reason=direct-pr-comment cmd=$_CMD_SHORT"
...
if echo "$COMMAND" | grep -qE 'codex\s+review'; then
  _hook_log "blocked" "reason=direct-codex-review cmd=$_CMD_SHORT"
